...
import DetailSupplierItem from "../../../lindsly-style-react/components/suppliers/DetailSupplierItem";
import call from "react-native-phone-call";
import { SupplierPageStyles as styles } from "../../../lindsly-style-react/styles/styles";
import {
  deleteSupplierRequest,
  fetchSupplierDataRequest,
  payToSupplierRequest,
  saveSupplierRequest,
} from "../../../common/api/Suppliers/SupplierActionsApi";
...
const SupplierPage = (props) => {
  ...
  const getData = useCallback(async () => {
    fetchSupplierDataRequest(myContext, event_id, supplierId, setSupplier);
  }, [event_id, supplierId]);
  useLayoutEffect(() => {
    setIsLoading(true);
    getData()
    ...
    return !isAddSupplier ? (
      <View style={styles.screen}>
        ...
        <View>
          <IconButton
            onPress={() => {
              payToSupplierRequest(myContext, event_id, supplierId, navigation);
            }}
            icon={"credit-card"}
            color={Colors.black}
            iconSize={18}
